  ...
  onRequest: async function (client, message, args) {
    ...
    collector.on("collect", i => {
        if(i.customId == "yardimmenusu") {
           if(i.values == "talent") {
                sehira.edit({embeds: [new genEmbed().setDescription(`${Data ?  Data.talentPerms ? Data.talentPerms.map(x => `\`${sistem.botSettings.Prefixs[0] + x.Commands + " <@pashqs/ID>"}\``).join("\n") : '' : ''}`)]})
           } else {
    ...
    collector.on("end", () => {
        ...
        \`${Data ?  Data.talentPerms ? Data.talentPerms.map(x => sistem.botSettings.Prefixs[0] + x.Commands + " <@pashqs/ID>").join("\n") : '' : ''}\``)], components: []})
      })
    }
};